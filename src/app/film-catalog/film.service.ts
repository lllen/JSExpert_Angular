...
export class FilmService {
  ...
  getPage(pageNumber : number, sortMethod : string){
    let temp = [];
    let j = 0;
    if(pageNumber*3 <= this.films.length){
      while(pageNumber){
        for(let i = 0; i < 3; i++,j++) {
          temp.push(this.films[j]);
        } pageNumber--;
      }
      if(sortMethod == "ASC"){
        return this.sort(temp, 1);
